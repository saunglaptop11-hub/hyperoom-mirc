...
export function createHyperoomRepository(client: HyperoomSupabaseClient): HyperoomRepository {
  return {
    ...
    async listPublicRooms() { const { error: archiveError } = await client.rpc("archive_inactive_rooms", { p_now: new Date().toISOString() }); if (archiveError) throw archiveError; const { data, error } = await client.from("rooms").select("*").eq("type", "public").in("lifecycle_status", ["active", "permanent"]).order("name"); if (error) throw error; return data.map(room); },
