...
export interface CreateRoomInput { name: string; type?: Database["public"]["Enums"]["room_type"]; description?: string | null; topic?: string | null }
...
export interface RealtimeHandlers {
  onMessage?: (message: HyperoomMessage) => void; onMessageUpdated?: (message: HyperoomMessage) => void;
  onReaction?: (reaction: HyperoomReaction, removed: boolean) => void; onMemberChange?: (member: HyperoomRoomMember, removed: boolean) => void;
  onRoomChange?: (room: HyperoomRoom) => void; onStatus?: (status: string) => void;
}
...
function room(row: Database["public"]["Tables"]["rooms"]["Row"]): HyperoomRoom { return { id: row.id, name: row.name, type: row.type, description: row.description, topic: row.topic, createdBy: row.created_by, createdAt: row.created_at, updatedAt: row.updated_at }; }
...
export function createHyperoomRepository(client: HyperoomSupabaseClient): HyperoomRepository {
  return {
    ...
    async createRoom(input, userId) { const id = await requireUserId(client, userId); const { data, error } = await client.from("rooms").insert({ name: input.name, type: input.type ?? "public", description: input.description ?? null, topic: input.topic ?? null, created_by: id }).select("*").single(); if (error) throw error; return room(data); },
    ...
    async updateRoomTopic(roomId, topic) { const { data, error } = await client.from("rooms").update({ topic: topic?.trim() || null }).eq("id", roomId).select("*").single(); if (error) throw error; return room(data); },
    async joinRoom(roomId, userId) { const id = await requireUserId(client, userId); const { data, error } = await client.from("room_members").upsert({ room_id: roomId, user_id: id, role: "member" }, { onConflict: "room_id,user_id", ignoreDuplicates: true }).select("*").maybeSingle(); if (error) throw error; if (data) { const actor = await this.getProfile(id); const name = actor?.username ?? id.slice(0, 8); const event = await client.from("messages").insert({ room_id: roomId, sender_id: id, kind: "system", event_type: "join", content: `*** ${name} joined the channel` }); if (event.error) throw event.error; return member(data); } const { data: existing, error: readError } = await client.from("room_members").select("*").eq("room_id", roomId).eq("user_id", id).single(); if (readError) throw readError; return member(existing); },
    async leaveRoom(roomId, userId) { const id = await requireUserId(client, userId); const current = await client.from("room_members").select("*").eq("room_id", roomId).eq("user_id", id).single(); if (current.error) throw current.error; if (current.data.role === "owner") throw new Error("Room owner cannot leave the room."); const actor = await this.getProfile(id); const name = actor?.username ?? id.slice(0, 8); const event = await client.from("messages").insert({ room_id: roomId, sender_id: id, kind: "system", event_type: "part", content: `*** ${name} left the channel` }); if (event.error) throw event.error; const { error } = await client.from("room_members").delete().eq("room_id", roomId).eq("user_id", id); if (error) throw error; },
    ...
    subscribePublicRooms(onChange) {
      const channel = client.channel("rooms:public");
      channel.on("postgres_changes", { event: "*", schema: "public", table: "rooms" }, () => { void this.listPublicRooms().then(onChange); });
