-- Final authoritative moderation function used by the client RPC.
-- This patch removes ambiguous RETURNS TABLE references and emits exact mIRC events.
...
begin
  ...
  insert into public.messages(room_id,sender_id,kind,event_type,content) values(p_room_id,auth.uid(),'system',v_action::public.message_event_type,case v_action when 'kick' then format('*** %s kicked %s from #%s%s',(select p.username from public.profiles p where p.id=auth.uid()),v_target.username,v_room.name,case when v_reason is null then '' else ' ('||v_reason||')' end) when 'ban' then format('*** %s banned %s from #%s%s',(select p.username from public.profiles p where p.id=auth.uid()),v_target.username,v_room.name,case when v_reason is null then '' else ' ('||v_reason||')' end) else format('*** %s unbanned %s from #%s',(select p.username from public.profiles p where p.id=auth.uid()),v_target.username,v_room.name) end);
