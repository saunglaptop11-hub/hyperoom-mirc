...
begin
  ...
  if public.is_room_banned(v_room.id) then return query select 'banned'::text, v_room.id, v_room.name, true, null::text, null::text, v_room.type, null::uuid, null::timestamptz, null::timestamptz; return; end if;
  if v_room.lifecycle_status = 'archived' then return query select 'archived'::text, v_room.id, v_room.name, v_room.is_locked, v_room.description, v_room.topic, v_room.type, v_room.created_by, v_room.created_at, v_room.updated_at; return; end if;
