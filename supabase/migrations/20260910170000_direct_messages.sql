-- Real 1:1 private messaging via DM rooms. No frontend-only messages.
...
begin
  ...
  where r.type='dm'
    and (select count(*) from public.room_members rm where rm.room_id=r.id)=2
  ...
  limit 1;
  if v_existing is not null then select * into v_room from public.rooms where id=v_existing; return v_room; end if;
  insert into public.rooms(name,type,description,created_by) values ('dm-'||substr(gen_random_uuid()::text,1,12),'dm','Private conversation',v_actor) returning * into v_room;
  update public.room_members set role='member' where room_id=v_room.id and user_id=v_actor;
  insert into public.room_members(room_id,user_id,role) values (v_room.id,v_target.id,'member');
  return v_room;
end;
$$;
