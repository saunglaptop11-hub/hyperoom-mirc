...
function canManage(profile: HyperoomProfile, room: HyperoomRoom, members: Props["members"]): boolean {
  return profile.systemRole !== "member" || room.createdBy === profile.id || members.some((m) => m.userId === profile.id && ["owner", "admin", "operator"].includes(m.role));
}
...
export function InvitationInbox({ repository, profile, onOpenRoom }: { repository: HyperoomRepository; profile: HyperoomProfile; onOpenRoom: (room: HyperoomRoom) => void }): React.JSX.Element {
  ...
  async function respond(item: HyperoomRoomInvitation, accept: boolean) { setBusy(item.id); try { await repository.respondInvitation(item.id, accept); if (accept && item.room) { await repository.joinRoom(item.room.id); onOpenRoom(item.room); } await refresh(); } finally { setBusy(null); } }
